...
final class ExactTileProcessor implements TileComputationStrategy {
    ...
    public TileComputationResult process(
        ...
        int validPixelCount = workerCount == 1
                ? traceRows(0, core.height(), request, metadata, bufferedValues, runConfig.exaggeration(), result, bvh, preparedLighting, lightingParameters)
                : traceRowsInParallel(workerCount, request, metadata, bufferedValues, runConfig.exaggeration(), result, bvh, preparedLighting, lightingParameters);
    ...
    private double tracePixel(
        ...
        if (lightingParameters.maxBounces() == 0) {
            for (int rayIndex = 0; rayIndex < lightingParameters.raysPerPixel(); rayIndex++) {
                ...
                total += tracePrimaryRay(originX, originY, originZ, originColumnIndex, direction, bvh, tracingContext);
            }
        } else {
            for (int rayIndex = 0; rayIndex < lightingParameters.raysPerPixel(); rayIndex++) {
                Vector3 direction = raySampler.primaryDirection(tileId, pixelIndex, rayIndex, lightingParameters);
